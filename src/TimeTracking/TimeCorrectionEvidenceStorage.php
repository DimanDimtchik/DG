...
/** Nachweisdateien zu Stempel-Korrekturen (WhatsApp-Screenshot, E-Mail-PDF, …). */
final class TimeCorrectionEvidenceStorage
{
    ...
    {
        return DG_ROOT . '/storage/time-corrections';
    }
    ...
    public static function tableReady(): bool
    {
        ...
        try {
            $r = Database::pdo()->query("SHOW TABLES LIKE 'dg_time_correction_attachments'");
    ...
    /**
     * Speichert Uploads zu einer Korrektur.
     *
     * @param array<string, mixed> $filesField $_FILES['evidence'] o.ä. (multi)
    ...
    public static function storeUploads(
        ...
        if ($correctionId < 1 || $contactId < 1) {
            return 0;
        }
        MigrationRunner::runPending();
        if (!self::tableReady()) {
            throw new RuntimeException('Anhang-Tabelle fehlt — Migration 099 ausführen.');
        }
        ...
        if (count($items) > self::MAX_FILES) {
            throw new InvalidArgumentException('Maximal ' . self::MAX_FILES . ' Nachweisdateien pro Korrektur.');
        }

        $dir = self::correctionDir($contactId, $correctionId);
        ...
        $ins = Database::pdo()->prepare(
            'INSERT INTO dg_time_correction_attachments
                (correction_id, contact_id, original_name, stored_name, mime, size_bytes, created_by)
             VALUES
                (:cid, :contact, :orig, :stored, :mime, :size, :by)'
        );
    ...
    /**
     * @param list<int> $correctionIds
     * @return array<int, list<array{id: int, original_name: string, mime: string, size_bytes: int}>>
     */
    public static function mapForCorrections(array $correctionIds): array
    {
        $ids = array_values(array_unique(array_filter(array_map('intval', $correctionIds), static fn (int $i): bool => $i > 0)));
        ...
        $stmt = Database::pdo()->prepare(
            "SELECT id, correction_id, original_name, mime, size_bytes
             FROM dg_time_correction_attachments
    ...
    /**
     * @return array{id: int, correction_id: int, contact_id: int, original_name: string, stored_name: string, mime: string}|null
     */
    public static function findById(int $attachmentId): ?array
    {
        ...
        MigrationRunner::runPending();
        $stmt = Database::pdo()->prepare(
            'SELECT id, correction_id, contact_id, original_name, stored_name, mime
             FROM dg_time_correction_attachments WHERE id = :id LIMIT 1'
        );
        ...
        return [
            'id' => (int) ($row['id'] ?? 0),
            'correction_id' => (int) ($row['correction_id'] ?? 0),
    ...
    public static function absolutePath(array $attachment): string
    {
        ...
        $correctionId = (int) ($attachment['correction_id'] ?? 0);
        $stored = (string) ($attachment['stored_name'] ?? '');
        if ($contactId < 1 || $correctionId < 1 || $stored === '' || str_contains($stored, '..')) {
            return '';
        }

        return self::correctionDir($contactId, $correctionId) . DIRECTORY_SEPARATOR . $stored;
    }

    public static function sendDownload(User $user, int $attachmentId): void
    {
        TimeCorrectionService::assertCanManage($user);
    ...
    private static function correctionDir(int $contactId, int $correctionId): string
    {
        return self::baseDir()
            . DIRECTORY_SEPARATOR . $contactId
            . DIRECTORY_SEPARATOR . $correctionId;
    }
