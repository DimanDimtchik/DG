...
{
    $path = DG_ROOT . '/storage/media/training/konten/' . $slug . '.meta.json';
    if (!is_file($path)) {
        return null;
    }
...
$modules = [
    ['Konten — Überblick', 'konten-ueberblick', 120],
    ['Konten — Hinweise', 'konten-hinweise', 180],
    ['Konten — Kontenübersicht', 'konten-kontenuebersicht', 120],
];
...
foreach ($modules as [$title, $slug, $fallback]) {
    ...
    $moduleIds[] = ensureModule(
        $pdo,
        $title,
        'media/training/konten/' . $slug . '.mp4',
        'media/training/konten/' . $slug . '.vtt',
        $sec
    );
...
if ($course === null) {
    $courseId = AcademyRepository::saveCourse([
        'title' => 'Konten',
        'slug' => $slug,
        'description' => 'Kontenrahmen durchsuchen, Hinweise nutzen und Salden in der Kontenübersicht prüfen.',
        'version' => '1.0',
    ...
    $pdo->prepare(
        'UPDATE dg_academy_courses SET description = :d WHERE id = :id'
    )->execute([
        'd' => 'Kontenrahmen durchsuchen, Hinweise nutzen und Salden in der Kontenübersicht prüfen.',
        'id' => $courseId,
    ]);
