...
{
    $path = DG_ROOT . '/storage/media/training/kichel/' . $slug . '.meta.json';
    if (!is_file($path)) {
        return null;
    }
...
$slug = 'kichel-ueberblick';
$meta = loadMeta($slug);
$sec = durationFromMeta($meta, 120);
$moduleId = ensureModule(
    $pdo,
    'Kichel — Schnelle Hilfe',
    'media/training/kichel/' . $slug . '.mp4',
    'media/training/kichel/' . $slug . '.vtt',
    $sec
);
echo "Modul: Kichel — Schnelle Hilfe ({$sec}s) id={$moduleId}\n";
...
if ($course === null) {
    $courseId = AcademyRepository::saveCourse([
        'title' => 'Kichel',
        'slug' => $courseSlug,
        'description' => 'Kichel — der CRM-Hilfe-Assistent: Fragen stellen, Stichworte nutzen, schnell den richtigen Weg finden.',
        'version' => '1.0',
